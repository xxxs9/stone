layui.config({
    ...
    $(function(){
        $(document).on("click",".layui-table tr",function(data) {
            console.log($('.layui-table tr'))
//这里写你的响应事件
        });
    })
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#gathering'
            , height: 415
            , url: $tool.getContext() + 'finance/gatheringList.do' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                  {type:'numbers',title:'序号',fixed: 'left'},
                  {field: 'receiveId', title: '应付单编号', width: '10%'}
        ...
        //为toolbar添加事件响应
        table.on('tool(gathering)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
