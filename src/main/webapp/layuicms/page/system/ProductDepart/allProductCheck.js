layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            ...
            , url: $tool.getContext() + 'check/pageList' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                   {type:'numbers',title:'序号',fixed: 'left'},
                   {field: 'formulaReachId', title: '领料单号'}
                  ,{field: 'checkUser', title: '检验人'}
                  ,{field: 'checkTime', title: '检验时间'}
                  ,{field: 'wasteId', title: '废料编号',/*templet:'#cd'*/}
                  ,{field: 'checkRemark', title: '产品类型'}
    ...
    function stepBack(id) {
        layer.confirm('确定撤回该验收单吗？', function (confirmIndex) {
            layer.close(confirmIndex);//关闭confirm
            //向服务端发送删除指令
            var req = {
                id: id
            ...
            $api.changeCheckState(req,function (data) {
        ...
        layer.confirm('确定将产品入库吗？', function (confirmIndex) {
            ...
            //todo(将产品发送给仓库)
            $api.sendToDepot(req,function (data) {
                layer.msg("产品库成功，请在仓库中查看",{time:1000},function(){
                    //重新加载表格
                    tableIns.reload();
                });
            });
