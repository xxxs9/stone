layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#menu-data'
            , height: 415
            , url: $tool.getContext() + 'productWaste/pageList' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                  {type:'numbers',title:'序号',fixed: 'left'},
                  {field: 'wasteNumber', title: '废料数量'}
                , {field: 'wasteTime', title: '报废时间'}
                , {field: 'produceFormulaId', title: '原配方编号'}
                , {field: 'state', title: '废料状态'}
                , {field: 'wasteRemark', title: '废料备注'}
    ...
    function editMenu(id){
        var index = layui.layer.open({
            title: "修改内容",
            type: 2,
            content: "editProductWaste.html?id="+id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
