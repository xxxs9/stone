layui.config({
    ...
    function defineTable() {
        tableIns = table.render({
            elem: '#menu-data'
            , height: 415
            , url: $tool.getContext() + '/create/pageList' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
                  {type:'numbers',title:'序号',fixed: 'left'},
                  {field: 'createUser', title: '主管姓名'}
                , {field: 'createTime', title: '创建时间'}
                , {field: 'employeeId', title: '员工号'}
    ...
    $(".usersAdd_btn").click(function () {
        var index = layui.layer.open({
            title: "添加新主管",
            type: 2,
            content: "addCreateUserInfo.html",
    ...
    function delMenu(id){
        layer.confirm('确认删除吗？', function (confirmIndex) {
            ...
            $api.delCreateUserInfo(req,function (data) {
    ...
    function editMenu(id){
        var index = layui.layer.open({
            title: "修改内容",
            type: 2,
            content: "editCreateUserInfo.html?id="+id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
