...
//监听事件
     ...
     * 初始化页面
     * */
    function getFormulaId(hh) {
        $api.getProduceFormulaDetail(null,function (res) {
            var data = res.data;
            if (data.length>0){
                var html='';
                for(var i=0;i<data.length;i++){
                    html += '<option value="'+data[i].produceFormulaId+'">'+data[i].produceFormulaId+'---'+data[i].other3+'</option>';
                }
    ...
    function getmaterialId(hh) {
        $api.getProduceFormulaDetail(null,function (res) {
            var data = res.data;
            if (data.length>0){
                var html='';
                for(var i=0;i<data.length;i++){
                    html += '<option value="'+data[i].other2+'">'+data[i].other2+'</option>';
                }
                $('#goodsName').append($(html));
                $('#goodsName').val(hh);
                form.render();
...
*/
    ...
    function initMenuInfo() {
        ...
        $api.getProduceFormulaDetailById(req,function (res) {
            ...
            getmaterialId(data.other2);
            getFormulaId(data.produceFormulaId);
 ...
 */
    ...
    form.on("submit(add)", function (data) {
        ...
        //请求
        var url = $tool.getContext()+'detail/upd';
        var req = {
            id:id1,
