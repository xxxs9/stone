    ...
    function defineTable() {
        ...
        table.on('tool(userFilter)', function (obj) { //注：tool是工具条事件名，test是table原始容器的属性 lay-filter="对应的值"
            ...
            if (layEvent === 'del') { //删除
                ...
                look(row.id);
            }else if (layEvent === 'depot') {//提交仓库发货
                depot(row.id);
            }else if (layEvent === 'accept') {//提交仓库发货
                accept(row.id);
            }
        });
    }
...
//仓库审核
    ...
    function depot(id) {
        var index = layui.layer.open({
            title: "编辑退货单",
            type: 2,
            content: "depotReturnGoodsAudit.html?id=" + id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    function accept(id){
        layer.confirm('确认查收吗？', function (confirmIndex) {
            layer.close(confirmIndex);//关闭confirm
            //向服务端发送撤回指令
            var req = {
                id: id
            ...
            $api.updateAccept(req,function (data) {
                layer.msg("查收成功",{time:1000},function(){
                    //obj.del(); //撤回对应行（tr）的DOM结构
                    //重新加载表格
                    tableIns.reload();
                });
    ...
    function editMarkerOrder(id) {
        var index = layui.layer.open({
            title: "编辑退货单",
            type: 2,
            content: "editReturnGoodsAudit.html?id=" + id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
    ...
    function look(id) {
        var index = layui.layer.open({
            title: "查看退货单",
            type: 2,
            content: "lookReturnGoodsAudit.html?id=" + id,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回用户列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
