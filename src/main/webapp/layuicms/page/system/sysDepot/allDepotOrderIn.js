layui.config({
    ...
    function init() {
        //初始化下拉框
        $api.GetDepotOrderInType(null,function (res) {
            var data = res.data;
            if(data.length > 0){
    ...
    function defineTable() {
        tableIns = table.render({
            ...
            ,elem: '#depot-data'
            , height: 'full'
            , url: $tool.getContext() + 'depotOrder/depotOrderInList.do' //数据接口
            , method: 'post'
            , page:true //开启分页
            , cols: [[ //表头
    ...
    //入库
    function storageDepotOrderIn(id){
        layer.confirm('确认入库吗？', function (confirmIndex) {
            ...
            $api.StorageInDepotOrderIn(req,function (data) {
                layer.msg("入库成功",{time:1000},function(){
                    //obj.del(); //删除对应行（tr）的DOM结构
                    //重新加载表格
                    tableIns.reload();
                });
    ...
    function auditDepotOrder(id,goodsId,goodsNumber){
        var index = layui.layer.open({
            title: "审核入库单",
            type: 2,
            content: "auditDepotOrderIn.html?id="+id+"&goodsId="+goodsId+"&goodsNumber="+goodsNumber,
            success: function (layero, index) {
                setTimeout(function () {
                    layui.layer.tips('点击此处返回入库单列表', '.layui-layer-setwin .layui-layer-close', {
                        tips: 3
